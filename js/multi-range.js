...
this.MultiRange.prototype = {
    ...
    events : {
        binding : function(){
            ...
            this.DOM.rangeWrap.addEventListener('touchstart', this.events.callbacks.onTouchStart.bind(this))
        ...
        callbacks : {
            onMouseDown : function(e){
                var target = e.target;
                if(e.target.className == "multiRange__range") return;
                if( !target ) return;
                ...
                if( nextElementSibling && nextElementSibling.querySelector('.multiRange__range__value') != null)
            ...
            onMouseMove : function(e){
                ...
                this.trigger('change', {idx:index, value:value, ranges:this.ranges, id:this})
            },
            onTouchStart : function(e){
                ...
                // set some variables (so percentages could be calculated on touchmove)
                var scopeClientRect = this.DOM.scope.getBoundingClientRect(),
                    previousElementSibling = this.DOM.currentSlice.previousElementSibling,
                    nextElementSibling =  this.DOM.currentSlice.nextElementSibling;
                ...
                // bind temporary events (save "bind" reference so events could later be removed)
                this.events.onTouchEndFunc = this.events.callbacks.onTouchEnd.bind(this);
                this.events.touchmoveFunc = this.events.callbacks.onTouchMove.bind(this);
            
                window.addEventListener('touchend', this.events.onTouchEndFunc)
                window.addEventListener('touchmove', this.events.touchmoveFunc)
            },
            
            onTouchEnd : function(e){
                this.DOM.currentSlice.classList.remove('grabbed');
                window.removeEventListener('touchmove', this.events.touchmoveFunc);
                window.removeEventListener('touchend', this.events.onTouchEndFunc);
                document.body.classList.remove('multiRange-grabbing');
            ...
            onTouchMove : function(e){
                ...
                var that = this,
                    ...
                    // minLeftPerc = this.settings.minRange/this.delta*100,
                    // minRightPerc = (this.delta - this.settings.minRange)/this.delta*100,
                    xPosScopeLeft = e.touches[0].clientX - this.state.offsetLeft, // the left percentage value
                    leftPrecentage = xPosScopeLeft / this.state.scopeWidth * 100,
                    prevSliceValue = this.ranges[+this.DOM.currentSlice.dataset.idx - 1],
                    nextSliceValue = this.ranges[+this.DOM.currentSlice.dataset.idx + 1];
